...
- *$"Case binary application with " n = 2 space (e_0 space e_1)$*
  ...
  - *Case $e_0 space e_1$ with $rho : r space e_0 space u$*
    ...
    *Proof Resulting from Rew:*\
    Given the outputs $r_e_0 : mono("relation") alpha -> tau$, $u : alpha -> tau$, and $p_e_0 : r_e_0 space e_0 space u$ of $mono("Rew")_rho (e_0)$ and the given outputs $r_e_1 : mono("relation") alpha$ and $p_e_1 : space r_e_1 space e_1 space e_1$ of $mono("Rew")_rho (e_1)$ the main invocation $mono("Rew"_rho) (e_0 space e_1)$ combines the proofs and carrier relations into a final proof of $r_tau (e_0 space e_1) space (u space e_1)$ where $r_tau$ is of type `relation` $tau$. We show the full Lean term including its type after the colon and all implicit arguments for all cases to highlight the well-formed composition of the proof terms:
...
- *Case `Rew` provides a proof for an identity rewrite*
  ...
  Using @theorem1 *(3)*, we can imply that `ORew` returns nothing (just an identity flag), and given $t = u$, the `imp_self` theorem creates a proof of $t <- t$ which is equal to the proof $p : r space t space t$ of `Rew` that is transformed to $t <- t$ in line 8 in @infersubp.
]

...
- *$"Case binary application with " n = 2 space (e_0 space e_1)$*
  ...
  - *Case $e_0 space e_1$ with $rho : r space e_0 space u$*
    ...
    *Proof Resulting from Rew:*\
    Given the outputs $r_e_0 : mono("relation") alpha -> tau$, $u : alpha -> tau$, and $p_e_0 : r_e_0 space e_0 space u$ of $mono("Rew")_rho (Psi, e_0)$ and the given outputs $r_e_1 : mono("relation") alpha$ and $p_e_1 : space r_e_1 space e_1 space e_1$ of $mono("Rew")_rho (Psi, e_1)$ the main invocation $mono("Rew"_rho) (Psi, e_0 space e_1)$ combines the proofs and carrier relations into a final proof of $r_tau (e_0 space e_1) space (u space e_1)$ where $r_tau$ is of type `relation` $tau$. We show the full Lean term including its type and all implicit arguments for all cases to highlight the well-formed composition of the proof terms:
  ...
  - *Case $e_0 space e_1$ with $rho : r space e_1 space u$*
    ...
    *Proof Resulting from ORew:*\
    ...
    With function argument unifying, we recursively invoke $mono("ORew"_rho) (Psi, e_0, r_tau : mono("relation") tau)$ and $mono("ORew"_rho) (Psi, e_1, r_tau)$. The first is flagged `identity` and thus returns no information. The latter results in a relation $r_e_1 : mono("relation") alpha$, and a proof $p_e_1 : r_e_1 space e_1 space u$. The algorithm then constructs a proof of $r_tau space (e_0 space e_1) space (e_0 space u)$:
...
- *Inductive case for $n + 1$ application subterms*
    ...
    $(Psi', r, e_0 ' space dots space e_n ', p : r space (e_0 space dots space e_n) space (e_0' space dots space e_n')) := mono("Rew"_rho) (Psi, e_0 space dots space e_n)$ implies:\ $(Psi'', r',e_0 ' space dots space e_n ', p' : r' space (e_0 space dots e_n) space (e_0 ' space dots space e_n ')) := mono("ORew"_rho) (Psi, e_0 space dots space e_n, r)$ \ given that $e_0 space dots space e_n != e_0 ' space dots space e_n '$ and $(Psi'', mono("identity")) := mono("ORew"_rho) (Psi, e_0 space dots space e_n, r)$ otherwise.
    ...
    $(Psi'', r', e_0 ' space dots space e_(n+1) ', p' : r' space (e_0 space dots space e_(n+1)) space (e_0 ' space dots space e_(n+1) ')) := mono("ORew"_rho) (Psi, e_0 space dots space e_(n+1), r)$ \ if $e_0 space dots space e_(n+1) != e_0 ' space dots space e_(n+1) '$ and $(Psi'', mono("identity")) := mono("ORew"_rho) (Psi, e_0 space dots space e_(n+1), r)$ otherwise.
  ...
  - *Case $e_0 space dots space e_n space e_(n+1)$ was not rewritten $e_0 space dots space e_n$ and $e_(n+1)$ doesn't unify with $rho_"lhs"$*
    ...
    *Proof Resulting from ORew*
    ...
    #theorem()[
      If $mono("InferRel") (mono("success") space mono("Rew")_rho (Psi, t : "Prop"), t)$ for $rho : r space t space u$ provides a proof of a rewrite from $t$ to $u$ of the form $t <- u$ containing metavariables, then the updated algorithm $mono("InferRel") (mono("ORew"_rho) (Psi, t, <-), t)$ also provides a proof of a rewrite from $t$ to $u$ of the form $t <- u$ containing metavariables.
    ] <theorem2>

...
- *Case `Rew` provides a proof for an identity rewrite*
  ...
  Using @theorem1 we can imply that `Subterm` returns nothing (just and identity flag) and given $t = u$ the `impl_self` theorem creates a proof of $t <- t$ which is equal to the proof $p : r space t space t$ of `Rew` that is transformed to $t <- t$ in line 8 in @infersub.
]

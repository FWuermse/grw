...
We saw that, in the average case, the improved algorithm generates significantly less constraints and leads to more concise proofs. In the following, we want to show that the two proposed algorithms for constraint generation provide the same rewrite proofs although with different metavariables.
...
#proof()[We show the premise by structural induction over the term $t$. The cases for lambda, pi, and arrow only differ by the additional status field. It suffices to show that applying a proof of identity ($?_r space t space t$) is equivalent to leaving $t$ unchanged *(3)*. The application case can be proven by induction over the application subterms $e_0 space dots space e_n$. We start with the base case $n = 2$ (a function with one argument) and have to consider the three cases (identity, success), (success, identity), and (identity, identity) under the assumption that the case (success, success) is not possible for a binary well-formed Lean application. This is because for any $f space e$, the types $f : sigma_0 -> dots -> sigma_n -> tau$ and $e : sigma_0$ cannot be the same, thus cannot both unify:

- *$"Case binary application with " n = 2 space (e_0 space e_1)$*

  - *Case $e_0 space e_1$ with $rho : r space e_0 space u$*

    This is the minimal case of a function application with one argument where the function unifies and the argument does not.

    *Proof Resulting from Rew:*\
    Given the outputs $r_e_0 : mono("relation") alpha -> tau$, $u : alpha -> tau$, and $p_e_0 : r_e_0 space e_0 space u$ of $mono("Rew")_rho (Psi, e_0)$ and the given outputs $r_e_1 : mono("relation") alpha$ and $p_e_1 : space r_e_1 space e_1 space e_1$ of $mono("Rew")_rho (Psi, e_1)$ the main invocation $mono("Rew"_rho) (Psi, e_0 space e_1)$ combines the proofs and carrier relations into a final proof of $r_tau (e_0 space e_1) space (u space e_1)$ where $r_tau$ is of type `relation` $tau$:
  ...
  - *Case $e_0 space e_1$ with $rho : r space e_1 space u$*
    ...
    *Proof Resulting from Rew:*\
    Given the outputs $r_e_0 : mono("relation") alpha -> tau$ and $p_e_0 : r_e_0 space e_0 space e_0$ of $mono("Rew")_rho (Psi, e_0)$ and the given outputs $r_e_1 : mono("relation") alpha$, $u : alpha$, and $p_e_1 : r_e_1 space e_1 space u$ of $mono("Rew")_rho (Psi, e_1)$ the `Rew` algorithm combines the proofs and carrier relations into a final proof of $r_tau : (e_0 space e_1) space (e_0 space u)$ where $r_tau$ is of type `relation` $tau$:
    ...
    *Proof Resulting from Subterm:*\
    ...
    With function argument unifying, we recursively invoke $mono("Subterm"_rho) (Psi, e_0, r_tau : mono("relation") tau)$ and $mono("Subterm"_rho) (Psi, e_1, r_tau)$. The first is flagged `identity` and thus returns no information. The latter results in a relation $r_e_1 : mono("relation") alpha$, and a proof $p_e_1 : r_e_1 space e_1 space u$. The algorithm then constructs a proof of $r_tau space (e_0 space e_1) space (e_0 space u)$:
  ...
  - *Case $e_0 space e_1$ where $rho$ does not unify given $rho : r space u space u'$*
    *Proof Resulting from Rew:*\
    ...
    Given the outputs $r_e_0 : mono("relation") alpha -> tau$ and $p_e_0 : r_e_0 space e_0 space e_0$ of $mono("Rew")_rho (Psi, e_0)$ and the given outputs $r_e_1 : mono("relation") alpha$ and $p_e_1 : space r_e_1 space e_1 space e_1$ of $mono("Rew")_rho (Psi, e_1)$, the `Rew` algorithm combines the proofs and carrier relations into a final proof of identity $r_tau (e_0 space e_1) space (e_0 space e_1)$ about the relation $r_tau$ of type `relation` $tau$:
    ...
    *Proof Resulting from Subterm*:\
      `Subterm` would simply return `identity` for such a rewrite exiting at line 37 in @subterm. As `Rew` returns proof of $r_tau space t space t$, this case also holds *(3)*.
...
- *Inductive case for $n + 1$ application subterms*
    ...
    There are four cases for the inductive step. Given the previous sequence was an identity rewrite, we must divide between another identity rewrite or a successful rewrite. If the previous sequence contains at least one successful rewrite, we also have the scenarios of another rewrite or a final identity. This differenciation is crucial to align with the Leading Identity Rewrite and Identity/Success Status optimisation.
  ...
  - *Case $e_0 space dots space e_n space e_(n+1)$ we rewrote $e_0 space dots space e_n$ and $e_(n+1)$ does not unify with $rho_"lhs"$*
    ...
    *Proof Resulting from Rew*
    ...
    This case is similar to the previous one as `Rew` doesn't change depending on how the proof looks. Let the relation be $r_e_n : mono("relation") (alpha_0 space dots space alpha_n)$. Let the proof be $p_e_n : r_e_n (e_0 ' space dots space e_n ') space (e_0 space dots space e_n)$, and $e_0 ' space dots space e_n '$ the rewritten term. The recursive invocation $mono("Rew")_rho (Psi, e_(n+1))$ outputs $r_e_(n+1) : mono("relation") alpha_(n+1)$, $p_e_(n+1) : r_e_(n+1) space e_(n+1) space e_(n+1)$. $r_tau$ is again the carrier relation for the resulting proof after combining the previous and current application rewrites to a proof of $r_tau (e_0 space dots space e_n space e_(n+1)) space (e_0 ' space dots space e_n ' space e_(n+1))$:
  ...
  - *Case $e_0 space dots space e_n space e_(n+1)$ where $e_0 space dots space e_n$ was not rewritten and $e_(n+1)$ unifies with $rho_"lhs"$*
    ...
    *Proof resulting from Rew*
    ...
    Let the identity proof obtained by $mono("Rew")_rho (Psi, e_0 space dots space e_n)$ be $p_e_n : r_e_n space (e_0 space dots space e_n) space (e_0 space dots space e_n)$. The recursive invocation $mono("Rew")_rho (Psi, e_(n+1))$ outputs the relation $r_e_(n+1) : mono("relation") alpha_(n+1)$, the updated term $e_(n+1) '$, and the proof $p_e_(n+1) : r_e_(n+1) space e_(n+1) space e_(n+1) '$. $r_tau$ is the carrier relation for the resulting proof after combining the previous and current application rewrites to a proof of $r_tau space (e_0 space dots space e_n space e_(n+1)) space (e_0 space dots space e_n space e_(n+1) ')$:
...
- *Case `Rew` returns a proof for $r space t space u$ with $t != u$ and `Subterm` infers $r$*
  ...
  In this case, the `InferRel` algorithm would match the `success` branch for both rewrite results (@theorem1) and return the `Subterm` result, which was already inferred by the assumption of this case, directly providing a proof $t <- u$. The `Rew` algorithm outputs a metavariable of type `relation Prop` which wrapped in the subrelation inference in line 8 at @infersubp. Both cases result in a proof for $t <- u$.

- *Case `Rew` returns a proof $r space t space u$ for $t != u$ and `Subterm` cannot infer $r$*

  In this case, both results are definitional equal and follow the same path in `InferRel`. Both exit at line 9 in @infersubp and provide a proof for $t <- u$.

- *Case `Rew` provides a proof for an identity rewrite*

  Using @theorem1 *(3)*, we can imply that `Subterm` returns nothing (just an identity flag), and given $t = u$, the `impl_self` theorem creates a proof of $t <- t$ which is equal to the proof $p : r space t space t$ of `Rew` that is transformed to $t <- t$ in line 8 in @infersubp.
]

With the proofs for @theorem1 and @theorem2, we have shown that for all proofs generated by the `Rew` algorithm for a given term $t$, the `Subterm` algorithm generates proofs of the same type and thus generates the same proofs. When looking at the proof terms, we can also observe that in every mentioned case the proof term and the constraints generated are of equal or smaller size in the `Subterm` algorithm. This assures us that even in the worst-case scenario we are guaranteed proofs with less (even if not significantly less) constraints.
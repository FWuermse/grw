...
The syntax for rewriting in Lean is the keyword `rewrite` followed by a list of rewrite theorems. By appending the `at` keyword in sequence with a hypothesis we can rewrite the hypothesis istead of the goal. Lean will then repeatedly replace all occurrences of the left-hand-side of the rewrite theorem in the current goal or specified hypothesis with its right-hand-side. If we want to perform a right-to-left rewrite for a certain rewrite theorem, we can state it with a right-to-left arrow in front of the argument. In our example we could also perform a rewrite ```lean rewrite [← h]``` to replace $1$ in the goal leaving us with $1 + 0 = 1 + 0$.
...
Generalised rewriting is the ability to replace subterm $t$ with $u$ when they are related by an equivalence relation $r$ other than equality and biimplication @sozeau:inria-00628904. In Type Theory, homogenious binary relations are terms of type $alpha -> alpha -> mono("Prop")$ where $alpha$ can be an arbitrary type and `Prop` is the type of propositions. Equality ($=$) is of type $alpha -> alpha -> mono("Prop")$ in Lean and the biimplication ($<->$) is of type $mono("Prop") -> mono("Prop") -> mono("Prop")$. We will also refer to relation types of $alpha -> alpha -> mono("Prop")$ as $mono("relation") alpha$.
...
In this work, we present the first complete reconstruction and formal description of the optimized generalised rewriting algorithm used in Coq, an algorithm that had remained undocumented and largely forgotten by its contributors. Building on this, we enhance Coq’s rewriting mechanism to match the capabilities described in the literature and formally prove the correctness of our improvements. We then bring these advancements to Lean 4, implementing the first version of generalised rewriting that maintains full consistency with Coq’s constraint generation while seamlessly integrating with Lean’s existing rewriting API. This work not only bridges a long-standing gap between Coq and Lean but also provides a rigorous foundation for future developments in proof automation. While our proof search currently falls short of Coq’s powerful typeclass resolution, we mitigate this by gracefully deferring unresolved subgoals to the user.

Notably, our Lean 4 implementation is the only rewriting library that achieves full consistency with Coq's constraint generation, making it the most reliable choice for formal proofs in Lean. TODO

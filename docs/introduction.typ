...
Rewriting in theorem provers is the process of replacing a subterm $t$ inside an expression with another term $u$. In Lean, rewriting is possible when the two terms are equal, $t = u$, or when the two terms are propositions and imply each other $p <-> q$. This allows us to replace a term in a goal we want to solve or inside one of our hypotheses when reasoning in Lean.
...
With generalised rewriting, we could solve the above example by replacing the occurrence of $[1, 1, 2]$ in the proof goal by the right-hand-side of the rewrite theorem $[2, 1]$. Lean 4 does not currently have support for generalised rewriting. A Lean proof where generalised rewriting was supported with a tactic `grewrite` would ideally look like the following:

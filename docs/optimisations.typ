...
In this example, we are left with only one relation metavariable that has to be assigned in a proof search, notably also the crucial one for this rewrite to succeed. With this improvement, we remove valuable time of the proof search assigning trivial `relation` and `Proper` metavariables and provide only the necessary ones.

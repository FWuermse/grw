...
The algorithm presented so far presents the essence of the rewriting implementation in Coq. We mentioned that for sequences of applications where the first argument, the function $f$, unifies with the rewriting theorem we do not need to explore the remaining arguments further. Although, this is how Coq treats rewrites on functions, it is inconsistent to the `Rew` algorithm where all occurrences are replaced due to the nature of treating all applications binary. To be consistent with the `Rew` algorithm we need to make a crucial change to our `ORew` algorithm that exceeds the capabilities of Coq's implementation.

In order to rewrite the function of an application in Coq, we have to define the according subrelation, pointwise relation, and relation constraints @sozeau:inria-00628904. Consider a rewrite theorem $rho : r space f space g$. $f$ and $g$ are of type $alpha -> mono("Prop") -> beta -> mono("Prop")$. Rewriting $f$ to $g$ in a term $f space a space (a = a) space b$ where $a$ is of type $alpha$ and $b$ of type $beta$ in Coq, the algorithm provides a proof e.g. with $(<-)$ as the desired relation for $f space a space (a = a) space b <- g space a space (a = a) space b$. In our `ORew` algorithm, we would enter the path starting at line 8 in @subterm and produce the according proof. However, when changing the term to $f space a space (f space a space (a = a) space b) space b$, Coq currently only provides a proof for $f space a space (f space a space (a = a) space b) space b <- g space a space (f space a space (a = a) space b) space b$. We can see that only the outer $f$ is rewritten by Coq's implementation, but the inner $f$ is unchanged. In the special case where $r$ is equality, Coq applies the algorithm for Leibniz Equality and replaces both occurrences of $f$.

To correct this inconsistency we propose a correction to the so-far by Coq inspired `ORew` algorithm. When we encounter a multiple nested occurrances of a function $f$ that unifies with the left-hand side of a rewrite theorem $rho$, we can replace the first occurrence as seen in lines 7-12 in @updatedalgo. Now the updated term $u$ contains one less occurrence of $f$, and $p$ is a proof of the rewrite over a relation $r$. Given those outputs we can then immediately rewrite again using the updated term $u$, infer $r$ early for $p$, and combine the proofs using transitivity of $r$. The inferred relation for $r$ can either be $<-$, $->$, or a metavariable that we assign during the proof search.

...
When only the left hand side of this minimal binary application unifies with $rho$:\
*Proof Resulting from Rew*:\
...
*Proof Resulting from Subterm*:\
  ...
  With function argument unifying we recursively invoke $mono("Subterm"_rho) (Psi, e_0, r_tau : "relation" tau)$ and $mono("Subterm"_rho) (Psi, e_1, r_tau)$. The first is flagged `identity` and thus returns no information. The latter results in a relation $r_e_1 : "relation" alpha$, and a proof $p_e_1 : r_e_1 space e_1 space u$. The algorithm then constructs the proof for the right-to-left implication $r_tau (e_0 space e_1) (e_0 space u)$.
...
Our assumption that $mono("Rew")_rho (Psi, e_0 space e_1)$ returning $r_tau space (e_0 space e_1) space (e_0 space u)$ implies $mono("Subrelation"_rho) (Psi, e_0 space e_1, r_tau)$ returning $r_tau space (e_0 space e_1) space (e_0 space u)$ holds.

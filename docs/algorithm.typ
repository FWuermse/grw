...
Eventually, we had to develop our own proof search that is tailored to the constraints generated by the algorithm. Aesop can only backtrack a single goal and all their resulting subgoals. For the constraints generated by the `Rew` algorithm however we have to backtrack accross multiple goals and their assigments. The reason for this is that the elements in the generated constraint set may share metavariables that can be solved easily when isolated but require more specific solutions when considering other constraints. The conjuction proof $?_p : mono("Proper") ((=) ==> ?_r ==> (<-)) space (and)$ mentioned in @Introduction, for instance, contains two metavariables. The metavariable $?_p$ must be assigned using the `proper` constructor, and $?_r : mono("relation Prop")$ must be assigned with a relation over `Prop`. Many assigments would be feasable for $?_r$, including the relations $->$, $and$, or $eq$. However, in a proof search that operates per goal it would be feasable to first solve $?_r$ with the implication relation. This would affect the metavariable $?_p$ to change to $mono("Proper") ((=) ==> (->) ==> (<-))$ which is not solvable without having instatiated the parameters of $?_p$. Thus, in such a case, we have to backtrack from the goal $?_p$ to a different assignment of $?_r$ and try another path.

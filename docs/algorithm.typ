...
Let us consider another example where the constraints become more difficult to solve. To visualize the proof better we will skip the individual steps and show a graph that represents the rewrite-sequence where each edge represents the return value passed by a recursive invocation of the `Rew` algorithm.

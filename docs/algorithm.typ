...
- Expr.proj is the notation for product types in Lean
...
Before we have a look at the rewriting algorithm, we want to define how Coq and also our Lean implementation uses morphisms to achieve successful rewrites. The following section specifies what we mean when a function is preserved by a relation.
...
Finally, we will have a look at the last case that is triggered whenever none of the above cases match. This is the case for constants such as `all`, `impl`, or simply for atoms that do not unify at the beginning of the `Rew` function. In this case, we construct another metavariable of type $"Proper" tau space ?_r t'$ that is treated as a hole at the bottom of the proof tree. We denote the creation of new metavariables using the $mono("mvar")$ function that takes a type and returns a metavariable of that type. It essentially represents a syntactic hole for a proof of an identity rewrite from $t$ to $t$ over a relation that is also a metavariable. This will always happen for this algorithm as we never specify the desired relation for the proof and generate metavariables whenever we do not know the relation.

...
In the second example, we can see a nested occurrence of `respectful`. When unfolding the definitions, we interpret it left-to-right as seen above and implicitly read parenthesis like so: $((=) ==> ((<->) ==> (<-)))$. We will refer to such nested respectful arrows as a respectful chain and to the arguments as elements of that chain in the remaining chapters. The reason we use two respectful arrows here is to match the desired type of the second `Proper` argument $(and)$. Conjunctions in Lean have the type $mono("Prop") -> mono("Prop") -> mono("Prop")$. Each relation in a respectful chain must be of type $mono("relation") alpha$ where $alpha$ is the corresponding type in the function that embodies the second `Proper` argument. For instance, the function $f : alpha_0 -> alpha_1$ requires a respectful chain $(a_0 : mono("relation") alpha_0) ==> (a_1 : mono("relation") alpha_1)$ for the term $mono("Proper") space (a_0 ==> a_1) space f$ to be well-formed. All three elements of the respectful chain above are relations over propositions. This matches the conjunction's type $and : mono("Prop") -> mono("Prop") -> mono("Prop")$. This always has to match in order to create a well-formed rewrite proof using this `Proper` framework.
...
The remaining cases leverage either the lambda or application case by converting the term $t$ slightly to fit in the scheme. Terms of non-dependent function types are transformed into an application with the constant `impl` as defined in @impl. This has the advantage that `const` definitions applied to arguments are represented as applications in Lean and thus reuse the already defined application case. We apply the same strategy for the case of a dependent function type that uses a local dependent function `all` as defined in @all.

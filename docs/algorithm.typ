...
= Naive Algorithm for Rewriting <PaperAlgo>
...
Before we have a look at the rewriting algorithm, we want to define how Coq and also our Lean implementation use morphisms to achieve successful rewrites. The following section specifies what we mean when a function is preserved by a relation.
...
The remaining cases leverage either the lambda or application case by converting the term $t$ slightly to fit in the scheme. Terms of non-dependent function types are transformed into the constant `impl` as defined in @impl for a function type. This has the advantage that `const` definitions applied to arguments are represented as applications in Lean and thus reuse the already defined application case. We apply the same strategy for the case of a dependent function type that uses a local dependent function `all` as defined in @all.
